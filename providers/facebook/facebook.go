...
type Option oauth2.AuthCodeOption

var (
	RerequestOption = oauth2.SetAuthURLParam("auth_type", "rerequest")
)
...
func (p *Provider) SetOptions(options ...Option) *Provider {
	p.Options = options
	return p
}
...
func (p *Provider) BeginAuth(state string) (goth.Session, error) {
	...
	authUrl := p.config.AuthCodeURL(state, options...)
	session := &Session{
		AuthURL: authUrl,
	}
	return session, nil
